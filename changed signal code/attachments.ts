...
import { join, normalize, basename, parse } from 'path';
...
export const makeSaveName = ({
  ...
  let i = 0;
  while (true) {
    if (!existsSync(join(directory, '${parsed.name}-${i}${parsed.ext}'))) {
      return '${parsed.name}-${i}${parsed.ext}';
    }
  }
};
